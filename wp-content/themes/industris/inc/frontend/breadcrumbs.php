<?php
// Breadcrumbs
if ( ! function_exists( 'industris_get_breadcrumbs' ) ) {
    function industris_get_breadcrumbs ( 
        $list_style = 'ul', 
        $list_id = 'breadcrumbs', 
        ...
        $theme = wp_get_theme();
        $text_domain =  $theme->get( 'industris' );
        // Open list
        $breadcrumb = '<' . $list_style . ' id="' . $list_id . '" class="' . $list_class . '">';
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', $text_domain) . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li><a href="' . home_url() . '">' . esc_html__('Home', $text_domain) . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        ...
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( 'Search', $text_domain ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( '404', $text_domain ) . '</li>';
        }
...
if ( ! function_exists( 'industris_breadcrumbs' ) ) {
    function industris_breadcrumbs (){

        if( industris_get_option('breadcrumbs') && !is_front_page() ){ echo industris_get_breadcrumbs(); } ?>
